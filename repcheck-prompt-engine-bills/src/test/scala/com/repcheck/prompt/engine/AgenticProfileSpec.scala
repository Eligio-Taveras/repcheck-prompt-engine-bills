...
class AgenticProfileSpec extends AnyFlatSpec with Matchers {

  private val json =
    """{
      |  "name": "cluster-concept-identification",
      |  "chain": [
      |    {"stage":"system","blockNames":["system-cluster-concept-identification"],"weight":1.0},
      |    {"stage":"custom","blockNames":["tool-use-follow-up"],"weight":1.0}
      |  ],
  ...
  "AgenticProfile" should "decode the staged chain, tools, and loop policy" in {
    decode[AgenticProfile](json) match {
      case Right(profile) =>
        profile.name shouldBe "cluster-concept-identification"
        profile.chain.map(_.stage) shouldBe List(PromptStage.System, PromptStage.Custom)
  ...
  it should "bridge to the shared PromptProfile via promptProfile (name + chain only)" in {
    decode[AgenticProfile](json).map(_.promptProfile) match {
      case Right(pp) =>
        pp.name shouldBe "cluster-concept-identification"
        pp.chain.flatMap(_.blockNames) shouldBe
  ...
  it should "carry a tokenBudget when present" in {
    ...
    decode[AgenticProfile](withBudget).map(_.loopPolicy.toLoopPolicy.tokenBudget) shouldBe Right(Some(4096))
  }
