 ...
 * the GCS description (name = stable key; schemas + examples stay codec-derived, so they can't drift). The list is
 * heterogeneous (`LlmTool[F, ?, ?]`) — each tool keeps its own In/Out.
 */
