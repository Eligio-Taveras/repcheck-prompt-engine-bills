 ...
 * object via the per-type naming methods ([[GcsObjectName.fragment]], [[AgenticTaskSpec.objectName]],
 * [[ToolBinding.descriptionObjectName]]) — each guarding against [[PromptObjectNameTooLong]]; fragments + task specs
...
final class GcsPromptLoader[F[_]: Sync](storage: Storage, bucket: String, prefix: String, version: String)
  ...
  def loadTaskSpec(taskSpecName: String): F[AgenticTaskSpec] =
    AgenticTaskSpec.objectName(taskSpecName, prefix, version).liftTo[F].flatMap { objectName =>
  ...
  def loadToolDescription(descriptionRef: String): F[String] =
    ToolBinding.descriptionObjectName(descriptionRef, prefix, version).liftTo[F].flatMap(readObject(_, descriptionRef))
