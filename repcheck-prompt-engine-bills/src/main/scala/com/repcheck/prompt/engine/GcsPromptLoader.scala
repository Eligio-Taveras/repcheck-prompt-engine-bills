 ...
 * JSON object via [[GcsObjectName]] (rejecting an over-long key with [[PromptObjectNameTooLong]]) and decodes it; a
 * missing object is a typed [[PromptObjectNotFound]] and a malformed object a typed
 * [[PromptFragmentParseFailed]]/[[PromptTaskSpecParseFailed]] — never a raw SDK or circe exception. Loads happen ONCE
 * at startup, so this path is intentionally retry-free — a transient GCS failure fails startup loudly and the
